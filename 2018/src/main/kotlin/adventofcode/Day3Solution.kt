...
with any other claim. If you can somehow draw attention to it, maybe the Elves will be able
...
class Day3Solution : Solution {
    ...
    private class Definition(defn: String) {

        companion object {
            private const val REGEX = """#(\d+) @ (\d+),(\d+): (\d+)x(\d+)$"""
        }

        val defnMatch = REGEX.toRegex().find(defn)!!.destructured.toList().map(kotlin.String::toInt)

        val id = defnMatch[0]
        val xpos = defnMatch[1]
        val ypos = defnMatch[2]
        val width = defnMatch[3]
        val height = defnMatch[4]
    }

    private val inputs = inputLines("inputs/day3.txt")
    private val grid = Array(1000) { IntArray(1000) }
    private val definitions = inputs.map { Definition(it) }

    init {
        definitions.forEach {
            for (i in it.xpos until it.xpos + it.width) {
                for (j in it.ypos until it.ypos + it.height) {
                    grid[i][j] += 1
                }
            }
        }
    ...
    override fun part2(): String {
        defnloop@ for (d in 0 until definitions.size) {
            val defn = definitions[d]
            for (i in defn.xpos until defn.xpos + defn.width) {
                for (j in defn.ypos until defn.ypos + defn.height) {
                    if (grid[i][j] != 1) continue@defnloop
                }
            }
