...
responds, "Actually, we've been working on a new kind of suit that would let him fit through tight
spaces like that. But, I heard that a few days ago, they lost the prototype fabric, the design plans,
...
class Day2Solution : Solution {
    ...
    private val ids = inputLines("inputs/day2.txt")

    override fun part1(): String {
        var twos = 0
        var threes = 0
        ids.map(String::toCharArray).forEach {
            val chars = it.sortedArray()
            val counts = mutableMapOf<Char, Int>()
            var token = chars[0]
            counts[token] = 1
            for (i in 1 until chars.size) {
                if (chars[i] == token) {
                    counts[token] = counts[token]!!.plus(1)
                } else {
                    token = chars[i]
                    counts[token] = 1
                }
            }
            if (counts.values.contains(2)) twos++
            if (counts.values.contains(3)) threes++
        }
        return (twos * threes).toString()
    }

    override fun part2(): String {
        for (pos in 0 until ids.first().length) {
            val seen = mutableMapOf<String, Boolean>()
            var choppedIds = ids.map {
                "${it.substring(0, pos)}${it.substring(pos + 1, it.length)}"
            }.forEach {
