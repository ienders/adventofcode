...
negative fuel should instead be treated as if it requires zero fuel; the remaining mass, if any, is instead handled by wishing really hard, which has no mass and is outside the scope of this calculation.
...
class Day1Solution : Solution(1) {
    ...
    private fun fuelRecursive(mass: Int): Int =
        fuel(mass).let { fuel ->
            fuel(fuel).let {
                fuel + it + if (it > 0) fuelRecursive(it) else 0
            }
        }

    private fun perform(mapper: (Int) -> Int): String =
        values.map(mapper).sum().toString()

    override fun part1() = perform(::fuel)
    override fun part2() = perform(::fuelRecursive)
