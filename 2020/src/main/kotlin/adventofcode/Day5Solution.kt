...
class Day5Solution : Solution(5) {
    ...
    private fun seek(instructions: String, inc: Char, dec: Char, max: Int): Int =
        instructions.fold(Pair(0, max)) { acc, instruction ->
            val step = (acc.second - acc.first + 1) / 2
            when (instruction) {
                inc -> Pair(acc.first, acc.second - step)
                dec -> Pair(acc.first + step, acc.second)
                else -> throw IllegalArgumentException()
            }
        }.first

    private fun seatID(pass: String): Int =
        seek(pass.take(7), 'F', 'B', 127) * 8 + seek(pass.takeLast(3), 'L', 'R', 7)

    override fun part1() = maxSeatID.toString()

    override fun part2() = (minSeatID..maxSeatID)
        .first { !seatIDs.contains(it) }
        .toString()
