...
class Day2Solution : Solution(2) {
    ...
    companion object {
        private val PATTERN = Regex("(\\d+)-(\\d+) (\\w+): (.*)")
    }

    private abstract class Spec(input: String) {
        protected val a: Int
        protected val b: Int
    ...
    private class CountSpec(input: String) : Spec(input) {
        override val valid = password.count { it == char } in a..b
    }

    private class PositionSpec(input: String) : Spec(input) {
        private val char1 = password[a - 1]
        private val char2 = password[b - 1]
        override val valid = (char1 == char || char2 == char) && char1 != char2
    }

    override fun part1() = inputLines
        .map { CountSpec(it) }
        .count { it.valid }
        .toString()

    override fun part2() = inputLines
        .map { PositionSpec(it) }
        .count { it.valid }
        .toString()
