...
class Day7Solution : Solution(7) {
    ...
    companion object {
        ...
        private val BAG_COUNT_REGEX = Regex("(\\d+) ([^,]+) bag")
        private val REGEX = Regex("^(.+) bags contain (.+)\\.$")
    }

    private val bags = inputLines.map {
        val (label, spec) = REGEX.find(it)!!.destructured
        val contents = mutableMapOf<String, Int>()
        if (spec != "no other bags") {
            BAG_COUNT_REGEX.findAll(spec).forEach { bagSpec ->
                val (count, bag) = bagSpec.destructured
                contents[bag] = count.toInt()
            }
        }
        label to contents
    }.toMap()

    private val directBagHolders = mutableMapOf<String, MutableSet<String>>().also {
        bags.forEach { (label, contents) ->
            contents.forEach { (subLabel, _) ->
                if (it.containsKey(subLabel)) {
                    it[subLabel]!!.add(label)
                } else {
                    it[subLabel] = mutableSetOf(label)
                }
            }
        }
    }

    private fun countEntryBagsTo(label: String): Int {
    ...
    override fun part1() = countEntryBagsTo(SHINY_GOLD).toString()

    override fun part2() = bagsInside(SHINY_GOLD).toString()
