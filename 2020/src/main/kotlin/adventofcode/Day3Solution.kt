...
class Day3Solution : Solution(3) {
    ...
    private fun toboggan(xstep: Int, ystep: Int = 1): Long =
        grid
            .filterIndexed { index, _ -> index % ystep == 0 }
            .fold(Pair(0, 0)) { (count, pos), row ->
                Pair(
                    count + (if (row[pos]) 1 else 0),
                    (pos + xstep) % row.size
                )
            }.first.toLong()
    ...
    override fun part2() =
        (toboggan(1) * toboggan(3) * toboggan(5) * toboggan(7) * toboggan(1, 2)).toString()
