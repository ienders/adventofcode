...
class Day4Solution : Solution(4) {
    ...
    private class Passport {
        ...
        companion object {
            ...
            private val HAIR_COLOR_REGEX = Regex("^#[0-9,a-f]{6}$")
        ...
        val allRequiredFieldsPresent: Boolean
            get() = fields.keys.containsAll(requiredFields)

        val valid: Boolean
            get() = allRequiredFieldsPresent &&
                validBirthYear &&
                validIssueYear &&
                validExpirationYear &&
                validHeight &&
        ...
        private fun yearFieldBetween(yearField: String, min: Int, max: Int): Boolean =
            fields.getValue(yearField).let {
                YEAR_REGEX.matches(it) && it.toInt() in min..max
            }

        private val validBirthYear: Boolean
            get() = yearFieldBetween(BIRTH_YEAR, 1920, 2002)

        private val validIssueYear: Boolean
            get() = yearFieldBetween(ISSUE_YEAR, 2010, 2020)

        private val validExpirationYear: Boolean
            get() = yearFieldBetween(EXPIRATION_YEAR, 2020, 2030)

        private val validHeight: Boolean
            get() = fields.getValue(HEIGHT).let {
                when (it.takeLast(2)) {
        ...
        private val validHairColor: Boolean
            get() = HAIR_COLOR_REGEX.matches(fields.getValue(HAIR_COLOR))

        private val validEyeColor: Boolean
            get() = EYE_COLOR_REGEX.matches(fields.getValue(EYE_COLOR))

        private val validPassportID: Boolean
            get() = PASSPORT_ID_REGEX.matches(fields.getValue(PASSPORT_ID))
