...
class Day10Solution : Solution(10) {
    ...
    private val adapterJoltages = inputLinesAsInts.sorted()
    private val differences = mutableMapOf<Int, Int>()
    private val runLengths = mutableMapOf<Int, Int>()
    ...
    init {
        ...
        adapterJoltages.forEach {
            ...
            if (diff == 1) {
                ...
                runLengths[run] = runLengths.getOrDefault(run, 0) + 1
                run = 0
            }

            last = it
        }
        differences[3] = differences.getOrDefault(3, 0) + 1
        runLengths[run] = runLengths.getOrDefault(run, 0) + 1
    }

    override fun part1() = differences.let {
        it.getOrDefault(1, 0) * it.getOrDefault(3, 0)
    }.toString()

    override fun part2() = runLengths.let {
        2.toDouble().pow(it.getOrDefault(2, 0)) *
        4.toDouble().pow(it.getOrDefault(3, 0)) *
        7.toDouble().pow(it.getOrDefault(4, 0))
    }.toLong().toString()
